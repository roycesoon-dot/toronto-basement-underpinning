---
title: "Interior Basement Waterproofing Cost in Ontario: 2026 Price Guide"
h1: "Interior Basement Waterproofing Cost in Ontario: What to Budget in 2026"
description: "Detailed breakdown of interior basement waterproofing costs in Ontario. Per linear foot rates, total project costs, component breakdown, and what is actually included in a real quote."
parentType: "hub"
parentSlug: "interior-waterproofing"
type: "informational"
angle: "cost"
stage: "informational"
targetKeyword: "interior basement waterproofing cost ontario"
secondaryKeywords: ["how much does basement waterproofing cost toronto", "interior waterproofing price ontario", "drainage channel installation cost"]
metaTitle: "Interior Basement Waterproofing Cost Ontario 2026 | Price Guide"
metaDescription: "Interior basement waterproofing costs in Ontario: $80 to $120 per linear foot standalone, $50 to $80 per linear foot during underpinning. Total project costs and full component breakdown."
publishDate: "2026-04-27"
updatedDate: "2026-04-27"
readingTimeMin: 7
polished: true
wordCountTarget: 2500
image: "/images/guides/guide-waterproofing-cost-hero.webp"
imageAlt: "Interior basement waterproofing installation in Toronto home"
contentImages:
  - url: "/images/guides/guide-waterproofing-cost-content.webp"
    alt: "Cost breakdown chart for interior waterproofing components"
    role: "inline"
internalLinksOut:
  - { targetSlug: "interior-waterproofing", targetType: "hub", anchorHint: "interior waterproofing" }
  - { targetSlug: "sump-pumps", targetType: "hub", anchorHint: "sump pump installation" }
  - { targetSlug: "contact", targetType: "page", anchorHint: "get a waterproofing quote" }
faqs:
  - question: "How much does interior basement waterproofing cost in Toronto?"
    answer: "Interior basement waterproofing in Toronto typically costs $8,000 to $20,000 for a standard detached or semi-detached home, depending on the perimeter length being waterproofed, the number of sump pits, and whether a battery backup pump is included. The per-linear-foot rate for a standalone waterproofing project runs $80 to $120. When waterproofing is done simultaneously with underpinning, the combined rate drops to $50 to $80 per linear foot for the waterproofing component."
  - question: "Is a permit required for interior basement waterproofing in Toronto?"
    answer: "A plumbing permit is required in Toronto when the sump pump discharge connects to the drainage system or when floor drains are modified. The permit process is straightforward and typically handled by the waterproofing contractor. Permit fees for plumbing work of this type run $300 to $600. The waterproofing installation itself (drainage channel, membrane) does not typically require a separate building permit if no structural changes are involved."
  - question: "What is the difference between a quote that includes everything and one that does not?"
    answer: "A complete interior waterproofing quote should include saw cutting, concrete removal and disposal, drainage channel installation, membrane installation, sump pit excavation and installation, primary sump pump supply and installation, concrete patching over the channel, and cleanup. Quotes that omit concrete disposal, the sump pump, or the discharge line plumbing can appear significantly cheaper until those items are added back."
draft: false
---

Interior basement waterproofing in Ontario costs $80 to $120 per linear foot for a standalone installation, or $50 to $80 per linear foot when done simultaneously with underpinning. A standard Toronto detached home with a 140-linear-foot perimeter, one sump pit, and a primary pump typically runs $15,000 to $18,000 installed. The wide quote range homeowners see — $6,000 to $25,000 — comes down to perimeter length, what components are included, and whether the project is bundled with other work.

## How Interior Waterproofing Is Priced

Interior waterproofing is priced per linear foot of drainage channel installed — the standard rate in Toronto is $80 to $120 per linear foot for a complete standalone installation, with the sump pit and pump priced separately as fixed items.

**Per linear foot of perimeter:** The most common pricing model. The rate covers the full installation per foot of drainage channel installed, including saw cutting, excavation, channel, membrane, and concrete. The sump pit and pump are typically priced separately as a fixed item.

**Lump sum per project:** Some contractors price by the project based on their assessment of scope. This approach makes comparison harder but is sometimes offered for smaller or simpler jobs.

The per-linear-foot rate for a standalone interior waterproofing project in Toronto currently runs **$80 to $120 per linear foot** for a complete installation. A typical Toronto detached home has a basement perimeter of 120 to 160 linear feet. Not all perimeters are waterproofed in every project; partial waterproofing (three walls only, or a specific problem area) is common.

## Cost by Component

| Component | Typical Cost |
|---|---|
| Saw cutting ($8 to $15/linear foot) | Included in per-LF rate |
| Drainage channel and gravel bed | Included in per-LF rate |
| Delta MS membrane installation | Included in per-LF rate |
| Concrete patching over channel | Included in per-LF rate |
| Sump pit excavation and liner | $800 to $2,000 per pit |
| Primary sump pump (0.5HP) | $600 to $1,200 installed |
| Battery backup pump (AGM) | $800 to $1,500 installed |
| Discharge line to exterior | $300 to $800 |
| Permits (plumbing) | $300 to $600 |
| Concrete disposal | $300 to $600 |

<ContentImage src="/images/guides/guide-waterproofing-cost-content.webp" alt="Cost breakdown chart for interior waterproofing components" />

## Total Project Cost: Typical Toronto Scenarios

### Scenario 1: Full Perimeter Waterproofing, Detached Home

A standard Toronto detached home (approximately 140 linear feet of perimeter to waterproof, single sump pit, primary pump, no battery backup):

- Drainage channel and membrane: 140 LF x $95/LF = $13,300
- Sump pit and liner: $1,200
- Primary pump: $900
- Discharge line: $500
- Permits and disposal: $800
- **Total: approximately $16,700**

### Scenario 2: Partial Perimeter Waterproofing, Semi-Detached

A Toronto semi-detached home waterproofing only three walls (party wall excluded, approximately 90 linear feet), with battery backup:

- Drainage channel and membrane: 90 LF x $95/LF = $8,550
- Sump pit and liner: $1,200
- Primary pump: $900
- Battery backup system: $1,200
- Discharge line: $500
- Permits and disposal: $700
- **Total: approximately $13,050**

### Scenario 3: Waterproofing During Underpinning

When waterproofing is installed simultaneously with underpinning, the per-linear-foot rate drops to **$50 to $80 per linear foot** because the trench is already open from the underpinning excavation. The mobilization cost is shared. For a 140-LF perimeter at $65/LF:

- Drainage channel and membrane: 140 LF x $65/LF = $9,100
- Sump pit and liner: $1,200
- Primary pump: $900
- Battery backup: $1,200
- Discharge line: $500
- **Total waterproofing component: approximately $12,900**
- **Savings vs standalone: approximately $3,800 to $5,600**

This is a significant reason why experienced contractors strongly recommend combining [waterproofing with underpinning](/interior-waterproofing/) rather than doing them sequentially.

## Factors That Drive Variance

**Perimeter length:** The single biggest cost driver. A small semi-detached in East York with an 80-LF perimeter will cost substantially less than a large detached home in North York with a 160-LF perimeter.

**Existing slab thickness:** Older Toronto homes often have 3 to 4-inch slabs. Newer homes may have 4 to 6-inch slabs. Thicker slabs require more saw cutting time and generate more concrete debris, adding $500 to $1,500 to the job.

**Number of sump pits:** Long perimeters may require two sump pits to maintain adequate drainage slope. Each pit adds $800 to $2,000 to the project cost.

**Battery backup inclusion:** Adding battery backup adds $800 to $1,500 but is highly recommended for finished basements and homes in lower-lying areas of the city.

**Tight access conditions:** Basements with low ceilings, narrow stair access, or partitioned layouts that restrict equipment movement add time and labour.

**Toronto vs smaller Ontario cities:** Toronto labour rates run 20 to 30 percent higher than smaller Ontario cities. The same project in London or Hamilton might cost $10,000 to $14,000 vs $14,000 to $18,000 in Toronto.

## What Is Included vs What Gets Added Later

A credible waterproofing quote will include all of the following without asking for extras:

**Included in a complete quote:**
- Saw cutting the perimeter channel
- Concrete removal and disposal (off-site)
- Drainage channel installation with gravel bed
- Dimple membrane on foundation walls
- Sump pit excavation and liner
- Primary sump pump supply and installation
- Discharge line routing to exterior
- Concrete patching over the channel
- Site cleanup and dust containment
- Plumbing permit

**Items sometimes excluded that you must confirm are included:**
- Battery backup pump
- Window well drains (if applicable)
- Discharge line buried through finished landscaping
- Inspection callbacks

<Callout>
Ask every waterproofing contractor to confirm in writing that their quote includes concrete removal and off-site disposal, the sump pump, and the discharge line run to daylight. These three items are the most commonly excluded line items in low-ball quotes that grow significantly once work begins.
</Callout>

## Permits and Their Cost

Waterproofing projects in Toronto require a plumbing permit when the sump pump discharge connects to any part of the plumbing or drainage system. The permit costs $300 to $600 and the inspection is straightforward. Your contractor should pull this permit and include the cost in their quote.

If the project is being done in combination with underpinning, the waterproofing scope may be covered under the underpinning building permit, or it may require a separate plumbing permit. Confirm this with your contractor.


For further context, read our guides on [Interior vs Exterior Waterproofing](/guide/interior-vs-exterior-basement-waterproofing/) and [Waterproofing During Underpinning](/guide/waterproofing-during-underpinning/).

Ready to get an accurate quote for your Toronto basement [waterproofing project](/interior-waterproofing/)? [Contact our team](/contact/) for a site assessment and detailed itemized quote.
